...
class CompressedSensing(LightningModule):
    ...
    def __init__(self, config):
        ...
        image_shape = config["exp_params"]["image_shape"]
        if num == 0:
            self.A = torch.zeros((image_shape[-1]*image_shape[-2]*image_shape[-3], num), device=self.device)
        else:
            self.A = np.sqrt(1/num)*torch.randn((image_shape[-1]*image_shape[-2]*image_shape[-3], num), device=self.device)
